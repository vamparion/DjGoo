...
class RelayState:
    ...
    async def remove_websocket_routes(self, websocket: web.WebSocketResponse) -> None:
        async with self.client_lock:
            stale = [
                route_id
                for route_id, route in self.clients.items()
                if route.websocket is websocket
            ]
...
async def host_socket(request: web.Request) -> web.WebSocketResponse:
    ...
    try:
        message = await asyncio.wait_for(
            websocket.receive(),
            timeout=HOST_HELLO_TIMEOUT_SECONDS,
        )
        ...
        await websocket.send_json(
            {"type": "host_ready", "protocol": 1, "room_id": room_id}
        )
        ...
        async for message in websocket:
            if message.type == WSMsgType.TEXT:
                ...
                if not isinstance(envelope, dict) or str(envelope.get("room_id") or "") != room_id:
                    await websocket.send_json(
                        {"type": "error", "code": "invalid_envelope"}
                    )
                    continue
                ...
                try:
                    ...
                    await websocket.send_json(
                        {"type": "error", "code": "host_disconnected"}
                    )
            elif message.type in {
                WSMsgType.ERROR,
                WSMsgType.CLOSE,
    ...
    while True:
        ...
        async with state.client_lock:
            stale = [
                ...
                if route.websocket.closed
                or now - route.created_at > STALE_ROUTE_SECONDS
            ]
            for route_id in stale:
...
def main() -> int:
    ...
    parser.add_argument(
        "--host",
        default=os.environ.get("DJGOO_RELAY_HOST", "0.0.0.0"),
    )
    parser.add_argument(
        "--port",
        type=int,
        default=int(os.environ.get("DJGOO_RELAY_PORT", "8080")),
    )
