...
def check_for_voice_update(project_root: Path, token: str | None = None) -> UpdateOffer | None:
    return select_voice_update(fetch_releases(token), read_installed_version(project_root))


def validate_voice_manifest(payload: object, offer: UpdateOffer) -> dict[str, object]:
    ...
    try:
        ...
        schema = 0
    if schema != 1:
        raise UpdateError(f"Unsupported recipient update manifest schema: {schema}")
    ...
    if str(payload.get("release_tag") or "") != offer.tag:
        raise UpdateError("The recipient update manifest tag does not match the release")
    if str(payload.get("bundle_asset") or "") != BUNDLE_ASSET_NAME:
        raise UpdateError("The recipient update manifest references an unexpected bundle")
...
def download_voice_update(
    ...
    try:
        ...
        raise UpdateError("The downloaded recipient update manifest is invalid") from exc
    manifest = validate_voice_manifest(payload, offer)

    download_asset(offer.bundle_asset, bundle_path, token, progress)
    if bundle_path.stat().st_size != int(manifest["bundle_size"]):
        raise UpdateError("The recipient update bundle size does not match its manifest")
    if _sha256(bundle_path) != str(manifest["bundle_sha256"]).lower():
        raise UpdateError("The recipient update bundle SHA-256 verification failed")
