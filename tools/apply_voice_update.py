...
def load_voice_manifest(path: Path) -> dict[str, object]:
    try:
        ...
        raise RuntimeError(
            f"Could not read recipient update manifest: {path}"
        ) from exc
    if not isinstance(payload, dict) or int(payload.get("schema", 0)) != 1:
        raise RuntimeError(
            "Unsupported or invalid recipient update manifest"
        )
    if payload.get("product") != PRODUCT_NAME:
        raise RuntimeError(
            "Recipient update manifest is for a different product"
        )
    files = payload.get("files")
    if not isinstance(files, list) or not files:
        raise RuntimeError(
            "Recipient update manifest does not list any files"
        )
    return payload
...
def restart_voice(root: Path, log) -> None:
    ...
    if not launcher.exists():
        log(
            f"Recipient restart skipped because "
            f"{LAUNCHER_NAME} is missing."
        )
        return
    try:
        subprocess.Popen(
